(function () {
  ...
  // Text setter for simple fields
  const setText = (id, text) => {
    const el = $(`#${id}`);
    if (el) el.textContent = text;
  };

  // Width setter for bar fills (expects 0..100)
  const setBar = (id, percent) => {
    const el = $(`#${id}`);
    if (el) el.style.width = `${percent}%`;
  };

  // Human-friendly byte formatter for memory numbers
  ...
  // Poll server stats for the dashboard, guard admin session, and render cards
  async function pollStats() {
    try {
      const res = await fetch('/api/stats');
      if (!res.ok) {
        // If admin session is missing/expired, return to admin login
  ...
  // Admin logout: clear server-side session then go back to admin login
  $('#logoutBtn').onclick = async () => {
    await fetch('/api/admin/logout', { method: 'POST' });
    window.location.href = '/admin';
  };

  // Restart server: forwards a POST to /api/admin/restart after confirmation
  ...
  // Kick off polling and refresh every 2 seconds
  pollStats();
